...
private:
	...
	FILE *GenerateDefaultConfig(const char *out);

	bool ReadColFile(const char fname[]);
	void InitDirectoryNames(std::string config_file);

	bool UsingSAMPObjects;
	bool UsingCustomObjects;

public:
	BuildManager(std::string &GTADirectory, std::string &ConfigFile, std::string &OutDirectory, bool SAMPObjects, bool CustomObjects);
