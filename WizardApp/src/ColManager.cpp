			...
			if (handle->gcount() == sizeof(fileItems))
			{
				...
				if (fileItems.numFaces)
				{
					...
					for (int i = 0; i < fileItems.numFaces; ++i)
					{
						...
						if (faceIndexes[i][0] > topIndex)
							topIndex = faceIndexes[i][0];
						if (faceIndexes[i][1] > topIndex)
							topIndex = faceIndexes[i][1];
						if (faceIndexes[i][2] > topIndex)
							topIndex = faceIndexes[i][2];
					}
